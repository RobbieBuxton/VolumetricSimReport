...
\begin{enumerate}[itemsep=-0.25em]
	\item Explain what the tracking system is.
	\item Explain why we need a tracking system.
	\item Explain what the tracking system is used for.
	\item Requirements (i.e needs to be high-resolution, low latency, high framerate, etc.)
\end{enumerate}
...
The purpose of this design choice is to make sure there is always a threading running the tracking models as these are by far the computationally most expensive part of the system. Using a multithreaded design does not reduce the latency (which we talk about more in evaluation) of the system but rather increases the throughput which in our case is the framerate of the application. As we are using a camera that runs at 30fps we need to be able to process an image every $\frac{100\text{ms}}{30\text{ms}} = 33 \text{ms} $.   \\
